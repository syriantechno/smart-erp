...
@section('subhead')
    <title>Cash Boxes - {{ config('app.name') }}</title>
@endsection
                    ...
                    <form method="POST" action="{{ route('accounting.cash-boxes.store') }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-6">
                                <x-base.form-label for="code">Code</x-base.form-label>
                                <x-base.form-input id="code" name="code" type="text" value="{{ old('code') }}" />
                            </div>
                            <div class="col-span-6">
                                ...
                                @forelse($cashBoxes as $box)
                                    <tr class="border-b border-slate-100 text-xs text-slate-700 dark:text-slate-300">
                                        <td class="py-2 pr-4">
                                            ...
                                            @if($box->code)
                                                <div class="text-[11px] text-slate-500">Code: {{ $box->code }}</div>
                                            @endif
                                        </td>
                                        <td class="py-2 pr-4">{{ $box->company->name ?? '-' }}</td>
                                        <td class="py-2 pr-4">{{ $box->account?->code }} - {{ $box->account?->name }}</td>
                                        <td class="py-2 pr-4">{{ $box->currency ?: '-' }}</td>
                                        <td class="py-2 pr-4">
                                            @if($box->is_active)
