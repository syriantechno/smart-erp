...
@section('subhead')
    <title>Evaluation Details - {{ $evaluation->employee->full_name ?? 'Employee' }}</title>
@endsection
...
@section('subcontent')
    ...
    <div class="intro-y mt-6 mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 no-print">
        <h2 class="text-2xl font-semibold text-royalDark flex items-center gap-2">
            <x-base.lucide icon="clipboard-check" class="w-7 h-7" />
            Evaluation Details
        </h2>
        <div class="flex items-center gap-2">
            <a href="{{ route('hr.employee-evaluations.index') }}" class="btn-royal btn-royal--outline btn-royal--sm">
            ...
            <button onclick="window.print()" class="btn-royal btn-royal--outline btn-royal--sm">
                <x-base.lucide icon="printer" class="w-4 h-4 mr-1" />
                Print
            </button>
            <a href="{{ route('hr.employee-evaluations.export-pdf', $evaluation) }}" class="btn-royal btn-royal--gold btn-royal--sm">
                <x-base.lucide icon="file-text" class="w-4 h-4 mr-1" />
                Export PDF
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-12 gap-6">
        {{-- Left Column - Employee Profile Card (Same as Employee Profile) --}}
        <div class="col-span-12 lg:col-span-4 2xl:col-span-3">
            ...
            <div class="intro-y box overflow-hidden">
                    ...
                    <div class="absolute -bottom-12 left-1/2 -translate-x-1/2">
                        <div class="h-24 w-24 rounded-full border-4 border-white dark:border-darkmode-600 overflow-hidden shadow-lg bg-white">
                            @if($evaluation->employee->profile_picture)
                                <img class="h-full w-full object-cover" src="{{ asset('storage/' . $evaluation->employee->profile_picture) }}" alt="{{ $evaluation->employee->full_name }}" />
                            @else
                ...
                <div class="pt-14 pb-5 px-5 text-center">
                    <a href="{{ route('hr.employees.show', $evaluation->employee) }}" class="text-xl font-semibold text-slate-800 dark:text-white hover:text-primary">
                        {{ $evaluation->employee->full_name ?? 'N/A' }}
                    </a>
                    <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">{{ $evaluation->employee->position ?? 'Employee' }}</p>
                    <div class="flex items-center justify-center gap-2 mt-2">
                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                            {{ $evaluation->employee->code ?? $evaluation->employee->employee_id ?? '-' }}
                        </span>
                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $evaluation->employee->is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' }}">
                            {{ $evaluation->employee->is_active ? 'Active' : 'Inactive' }}
                        </span>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-200/60 dark:border-darkmode-400 px-5 py-5">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate max-w-[120px]">{{ $evaluation->employee->department->name ?? '-' }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate max-w-[120px]">{{ $evaluation->employee->position ?? '-' }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="tel:{{ $evaluation->employee->phone }}" class="text-sm font-medium text-primary hover:underline">{{ $evaluation->employee->phone }}</a>
                        </div>
                        @endif
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="mailto:{{ $evaluation->employee->email }}" class="text-sm font-medium text-primary hover:underline truncate max-w-[120px]">{{ $evaluation->employee->email }}</a>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="border-t border-slate-200/60 dark:border-darkmode-400 px-5 py-5">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ $evaluation->evaluated_at ? $evaluation->evaluated_at->format('M d, Y') : '-' }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ $evaluation->evaluator->name ?? 'System' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="intro-y box p-5 mt-5">
                <h4 class="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-4 flex items-center gap-2">
                    <x-base.lucide icon="award" class="w-4 h-4" />
                    Overall Rating
                </h4>
                @php
                ...
                <div class="text-center py-4">
                    <div class="text-6xl font-bold {{ $evaluation->overall_rating >= 8 ? 'text-green-600' : ($evaluation->overall_rating >= 5 ? 'text-amber-600' : 'text-red-600') }}">
                        {{ $evaluation->overall_rating }}<span class="text-2xl text-slate-400">/10</span>
                    </div>
                    <div class="flex items-center justify-center gap-1 mt-3">
                        @for($i = 1; $i <= 10; $i++)
                            <x-base.lucide icon="star" class="w-5 h-5 {{ $i <= $evaluation->overall_rating ? $overallStarColor : 'text-slate-300' }}" />
                        @endfor
                    </div>
                    <div class="mt-3">
                        @if($evaluation->overall_rating >= 8)
                        ...
                        @elseif($evaluation->overall_rating >= 5)
        ...
        {{-- Right Column - Criteria Details --}}
        <div class="col-span-12 lg:col-span-8 2xl:col-span-9">
            <div class="intro-y box p-5">
                <h4 class="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-4 flex items-center gap-2">
                    <x-base.lucide icon="list-checks" class="w-4 h-4" />
                    Evaluation Criteria
                </h4>
                            ...
                            <div class="p-4 rounded-lg bg-slate-50 dark:bg-darkmode-600/50">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="font-medium text-slate-700 dark:text-slate-300">
                                        {{ $item->criterion->name ?? 'Unknown Criterion' }}
                                    </span>
                                    <span class="font-bold {{ $item->score >= 8 ? 'text-green-600' : ($item->score >= 5 ? 'text-amber-600' : 'text-red-600') }}">
                                        {{ $item->score }}/10
                                    </span>
                                </div>
                                <div class="flex items-center gap-1">
                                    @for($i = 1; $i <= 10; $i++)
                                        <x-base.lucide icon="star" class="w-4 h-4 {{ $i <= $item->score ? $starColor : 'text-slate-300' }}" />
                                    @endfor
                                </div>
                                @if($item->notes)
                ...
                @else
                    <div class="text-center py-8 text-slate-500">
                        <x-base.lucide icon="clipboard-x" class="w-12 h-12 mx-auto mb-3 text-slate-300" />
                ...
                @if($evaluation->comments)
                    <div class="mt-6 pt-6 border-t border-slate-200/60 dark:border-darkmode-400">
                        ...
                        <div class="p-4 rounded-lg bg-slate-50 dark:bg-darkmode-600/50">
                            <p class="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-line">{{ $evaluation->comments }}</p>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-200/60 dark:border-darkmode-400">
                        ...
                        <div class="grid grid-cols-3 gap-4 text-center">
                            <div class="p-4 rounded-lg bg-green-50 dark:bg-green-900/20">
                                <div class="text-2xl font-bold text-green-600">{{ $evaluation->items->where('score', '>=', 8)->count() }}</div>
                            ...
                            <div class="p-4 rounded-lg bg-amber-50 dark:bg-amber-900/20">
                                <div class="text-2xl font-bold text-amber-600">{{ $evaluation->items->whereBetween('score', [5, 7])->count() }}</div>
                                <div class="text-xs text-amber-700 dark:text-amber-400">Good</div>
                            </div>
                            <div class="p-4 rounded-lg bg-red-50 dark:bg-red-900/20">
                                <div class="text-2xl font-bold text-red-600">{{ $evaluation->items->where('score', '<', 5)->count() }}</div>
    ...
    <div class="print-only" id="print-content">
        <div style="text-align: center; border-bottom: 3px solid #1e293b; padding-bottom: 15px; margin-bottom: 20px;">
            ...
            <p style="font-size: 12px; color: #64748b; margin: 5px 0 0 0;">{{ config('app.name') }} | {{ $evaluation->evaluated_at ? $evaluation->evaluated_at->format('F d, Y') : now()->format('F d, Y') }}</p>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            <tr>
                ...
                <td style="padding: 8px; border: 1px solid #cbd5e1; width: 25%;">{{ $evaluation->employee->full_name ?? 'N/A' }}</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold; width: 25%;">Employee ID</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1; width: 25%;">{{ $evaluation->employee->employee_number ?? $evaluation->employee->code ?? '-' }}</td>
            </tr>
            <tr>
                <td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold;">Department</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1;">{{ $evaluation->employee->department->name ?? '-' }}</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold;">Position</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1;">{{ $evaluation->employee->position ?? '-' }}</td>
            </tr>
            <tr>
                <td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold;">Evaluated By</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1;">{{ $evaluation->evaluator->name ?? 'System' }}</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold;">Evaluation Date</td>
                <td style="padding: 8px; border: 1px solid #cbd5e1;">{{ $evaluation->evaluated_at ? $evaluation->evaluated_at->format('F d, Y') : '-' }}</td>
            </tr>
        </table>
        ...
        @endphp
        <div style="text-align: center; padding: 20px; border: 2px solid {{ $ratingColor }}; border-radius: 8px; margin-bottom: 20px;">
            <div style="font-size: 48px; font-weight: bold; color: {{ $ratingColor }};">{{ $evaluation->overall_rating }}/10</div>
            <div style="font-size: 18px; margin-top: 5px;">
                @for($i = 1; $i <= 10; $i++)
                    <span style="color: {{ $i <= $evaluation->overall_rating ? '#1e293b' : '#cbd5e1' }};">★</span>
                @endfor
            </div>
            <div style="font-size: 14px; font-weight: bold; color: {{ $ratingColor }}; margin-top: 10px;">{{ $ratingLabel }}</div>
        </div>
                ...
                <tbody>
                    @foreach($evaluation->items as $index => $item)
                        ...
                        <tr>
                            ...
                            <td style="padding: 8px; border: 1px solid #cbd5e1;">{{ $item->criterion->name ?? 'Unknown' }}</td>
                            <td style="padding: 8px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold; color: {{ $itemColor }};">{{ $item->score }}/10</td>
                            <td style="padding: 8px; border: 1px solid #cbd5e1;">
                                @for($i = 1; $i <= 10; $i++)
                                    <span style="color: {{ $i <= $item->score ? '#1e293b' : '#cbd5e1' }};">★</span>
                                @endfor
                            </td>
                        </tr>
                    @endforeach
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <tr>
                    <td style="width: 33%; padding: 15px; text-align: center; border: 2px solid #16a34a;">
                        <div style="font-size: 24px; font-weight: bold; color: #16a34a;">{{ $evaluation->items->where('score', '>=', 8)->count() }}</div>
                    ...
                    <td style="width: 33%; padding: 15px; text-align: center; border: 2px solid #d97706;">
                        <div style="font-size: 24px; font-weight: bold; color: #d97706;">{{ $evaluation->items->whereBetween('score', [5, 7])->count() }}</div>
                    ...
                    <td style="width: 33%; padding: 15px; text-align: center; border: 2px solid #dc2626;">
                        <div style="font-size: 24px; font-weight: bold; color: #dc2626;">{{ $evaluation->items->where('score', '<', 5)->count() }}</div>
