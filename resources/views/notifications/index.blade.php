...
@section('subhead')
    <title>{{ __('Notifications') }} - {{ config('app.name') }}</title>
    <style>
        .notification-card {
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-slate-800 dark:text-white">{{ __('Notifications') }}</h1>
                    <p class="text-slate-500 mt-1">{{ __('Stay updated with your latest activities') }}</p>
                </div>
            ...
            <div class="flex items-center gap-4 mb-6" id="notification-stats">
                <div class="stats-badge total">
                    ...
                    <span id="total-count">0</span> {{ __('Total') }}
                </div>
                <div class="stats-badge unread">
                    <i data-lucide="bell-ring" class="w-4 h-4"></i>
                    <span id="unread-count">0</span> {{ __('Unread') }}
                </div>
                <div class="stats-badge read">
                    <i data-lucide="bell-off" class="w-4 h-4"></i>
                    <span id="read-count">0</span> {{ __('Read') }}
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <button class="filter-tab" data-filter="read">{{ __('Read') }}</button>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-darkmode-600 dark:border-darkmode-400">
            <div id="notifications-container">
                ...
                <div class="p-8 text-center">
                    ...
                    <p class="text-slate-500 mt-4">{{ __('Loading notifications...') }}</p>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('mark-all-read-btn').addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success) {
                showToast('success', '{{ __("Success") }}', '{{ __("All notifications marked as read") }}');
                loadNotifications(currentPage);
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showToast('error', '{{ __("Error") }}', '{{ __("Failed to mark notifications as read") }}');
        });
    });
    ...
    document.getElementById('delete-all-btn').addEventListener('click', function() {
        const doDeleteAll = () => {
            ...
            .then(data => {
                if (data.success) {
                    showToast('success', '{{ __("Success") }}', '{{ __("All notifications deleted") }}');
                    loadNotifications(currentPage);
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showToast('error', '{{ __("Error") }}', '{{ __("Failed to delete notifications") }}');
            });
        };

        if (typeof window.confirmDelete === 'function') {
            window.confirmDelete('all notifications', doDeleteAll);
        } else if (confirm('{{ __("Are you sure you want to delete all notifications?") }}')) {
            doDeleteAll();
        }
    });
        ...
        .catch(error => {
            ...
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800">{{ __('Error loading notifications') }}</h3>
                    <p class="text-slate-500 mt-2">{{ __('Please try again later') }}</p>
                </div>
            `;
        ...
        if (!notifications || notifications.length === 0) {
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-white">{{ __('No notifications') }}</h3>
                    <p class="text-slate-500 mt-2">{{ __("You're all caught up! Check back later for updates.") }}</p>
                </div>
            `;
            if (typeof lucide !== 'undefined') lucide.createIcons();
            return;
                        ...
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="flex items-center gap-2" onclick="event.stopPropagation()">
                                ${!notification.is_read ? `
                                    <button onclick="markAsRead(${notification.id})" class="action-btn read" title="{{ __('Mark as read') }}">
                                        <i data-lucide="check" class="w-4 h-4"></i>
                                    </button>
                                ` : ''}
                                <button onclick="deleteNotification(${notification.id})" class="action-btn delete" title="{{ __('Delete') }}">
                                    <i data-lucide="trash-2" class="w-4 h-4"></i>
                                </button>
                            </div>
                        </div>
    ...
    window.deleteNotification = function(id) {
        ...
        } else if (confirm('{{ __("Are you sure you want to delete this notification?") }}')) {
            doDelete();
        }
    };
    ...
    function formatTime(dateString) {
        ...
        if (diffInMinutes < 60) return `${diffInMinutes}{{ __("m ago") }}`;

        const diffInHours = Math.floor(diffInMinutes / 60);
        if (diffInHours < 24) return `${diffInHours}{{ __("h ago") }}`;

        const diffInDays = Math.floor(diffInHours / 24);
        if (diffInDays < 7) return `${diffInDays}{{ __("d ago") }}`;
